		...
		while ( have_posts() ) : the_post();
			...
			if($one_day_event == true){
				...
				if($start_month == $end_month){
					$date = $start_date->format('F')." ".$start_date->format('j'). ' to '.$start_date->format('F')." ". $end_date->format('j').", ".$start_date->format('Y');
					$month = $start_month;
					$another_time = '';
				} else {
					$date = $start_date->format('F')." ".$start_date->format('j'). ' to '.$end_date->format('F')." ". $end_date->format('j').", ".$start_date->format('Y');
					$month = $start_month;
						...
						<div class="eventCTA">
							<div class="row">
								...
								if($one_day_event == true){
									...
									$google_date = $start_date->format('Ymd');
									$google_end_date = $end_date->format('Ymd');
									$google_string = 'TEMPLATE&text='.$title.'&dates='.$google_date.'T'.$start_time1.'/'.$google_end_date.'T'.$end_time1.'&details=&location='.$location;
								}
									
									//$google_start_date = $start_date->format('Ymd');
									//$google_end_date = $end_date->format('Ymd');
